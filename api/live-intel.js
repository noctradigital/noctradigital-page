// Vercel Serverless Function — /api/live-intel
// Converted from live-intel-backend/server.js (no Express/cors/dotenv needed)

// ════════════════════════════════════════════════════
// MODULE-LEVEL CACHE  (survives warm re-invocations)
// ════════════════════════════════════════════════════
...
function pointByCode(code) {
  return EU_POINT_BY_CODE.get(code) || pickPoint(code);
}
...
const CERT_FEEDS = [
  { source: "CERT-EU",  country: "EU",          countryCode: "BE", urls: ["https://cert.europa.eu/publications", "https://cert.europa.eu"] },
  { source: "CERT-FR",  country: "France",       countryCode: "FR", urls: ["https://www.cert.ssi.gouv.fr/feed/", "https://www.cert.ssi.gouv.fr/alerte/feed/"] },
  { source: "NCSC-NL",  country: "Netherlands",  countryCode: "NL", urls: ["https://www.ncsc.nl/rss", "https://www.ncsc.nl"] },
  { source: "CERT-PL",  country: "Poland",       countryCode: "PL", urls: ["https://cert.pl/en/feed/", "https://cert.pl/feed/"] },
  { source: "NCSC-UK",  country: "United Kingdom",countryCode: "GB", urls: ["https://www.ncsc.gov.uk/api/1/services/v1/report-rss-feed.xml", "https://www.ncsc.gov.uk/rss.xml"] },
];
...
async function getNvdCves() {
  try {
    ...
    const items = vulns.map(entry => {
      ...
      return {
        id: `nvd-${id}`, source: "NVD", type: "cve",
        title: id, summary: desc.slice(0, 220),
        severity: severityFromCvss(score), cvss: score, vendor,
        country: null, countryName: null, timestamp: published,
...
async function getHibpBreaches() {
  try {
    const { json, ms } = await fetchJson("https://haveibeenpwned.com/api/v3/breaches");
    ...
    const items = sorted.map(breach => {
      ...
      return {
        id: `hibp-${name}`, source: "HIBP", type: "breach",
        title: `BREACH: ${title}`,
        summary: `${domain || "n/a"} \u2022 ${count.toLocaleString()} records \u2022 ${breachDate} \u2022 ${classes.slice(0, 120)}`,
        severity, pwnCount: count,
        country: point.code, countryName: point.country,
    ...
    return { items: [], responseMs: 0, ok: false, error: err.message };
  }
}
...
async function getCertAdvisories() {
  const results = await Promise.all(
    CERT_FEEDS.map(async feed => {
      ...
      const items = mappedItems.length ? mappedItems : [{
        id: `adv-${feed.source}-portal`,
        source: feed.source, type: "advisory",
        title: `${feed.source} advisory portal`,
        summary: `Live feed temporarily unavailable. Open ${feed.source} official portal for latest advisories.`,
        severity: "LOW",
        country: feed.countryCode, countryName: feed.country,
        timestamp: nowIso(),
      ...
      return { source: feed.source, ok: fetched.ok, responseMs: fetched.ms, items };
    })
  );
...
function buildCountryActivity(events) {
  ...
  for (const evt of events) {
    ...
    const rec = byCode.get(code) || {
      code, country: countryNameByCode(code),
      x: pointByCountryCode(code).x, y: pointByCountryCode(code).y,
      eventCount: 0, score: 0,
    };
    rec.eventCount += 1;
...
function buildCertMarkers() {
  return CERT_MARKERS.map((m, idx) => {
    ...
    return {
      id: `cert-${idx}-${m.code}`, code: m.code, cert: m.cert, url: m.url,
      x: point.x, y: point.y, city: point.city,
...
function buildRadarBlips(events) {
  return events.slice(0, 12).map(evt => ({
    id: evt.id, x: evt.radar?.x ?? 340, y: evt.radar?.y ?? 300,
    severity: evt.severity, label: evt.radar?.city || evt.country || "EU",
    source: evt.source, title: evt.title,
  }));
}
...
function buildCommunicationLog(events) {
  return events
    ...
    .slice(0, 30)
    .map(evt => ({
      id: evt.id, timestamp: evt.timestamp,
      line: `[${evt.source}] ${evt.title}`,
      severity: evt.severity, source: evt.source, url: evt.url,
    }));
}
...
function computeStats(events, responseTimes, sourceHealth) {
  const recent = events.filter(e => {
    ...
    return !Number.isNaN(d.getTime()) && hoursAgo(d) <= 168;
  }).length;
...
async function buildIntelPayload() {
  ...
  return {
    ...
    map: {
      certMarkers, countryActivity,
      countryActivitySource: "Shadowserver-modeled",
      notes: "Country activity is modeled from current event density.",
    },
    panels: {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (cache.intel && age < CACHE_TTL_MS) {
      ...
      res.status(200).end(JSON.stringify({ ...cache.intel, cache: { hit: true, ageMs: age } }));
      return;
    }
    ...
    res.setHeader("Content-Type", "application/json");
    res.status(200).end(JSON.stringify({ ...payload, cache: { hit: false, ageMs: 0 } }));
  } catch (err) {
    res.setHeader("Content-Type", "application/json");
    res.status(500).end(JSON.stringify({ ok: false, error: err.message, generatedAt: nowIso() }));
  }
}
